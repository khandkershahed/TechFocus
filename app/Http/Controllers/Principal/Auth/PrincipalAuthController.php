...
// if ($request->has('addresses')) {
//     foreach ($request->addresses as $addressData) {
//         $countryName = $addressData['country_name'] ?? null; // Only country name

//         $principal->addresses()->create([
//             'type' => $addressData['type'] ?? 'HQ',
//             'line1' => $addressData['line1'] ?? null,
//             'line2' => $addressData['line2'] ?? null,
//             'country_name' => $countryName, // store only country name
//             'city' => $addressData['city'] ?? null,
//             'state' => $addressData['state'] ?? null,
//             'postal' => $addressData['postal'] ?? null,
//         ]);
...
public function register(Request $request)
{
    try {
        ...
        $request->validate([
            ...
            'website_url' => 'nullable|url|max:255',
            'hq_city' => 'nullable|string|max:255',
            'country_id' => 'nullable|exists:countries,id', // change here
            'name' => 'required|string|max:255',
            'email' => 'required|email|unique:principals,email',
            'password' => 'required|confirmed|min:6',
            ...
            'addresses.*.line1' => 'required|string|max:255',
            'addresses.*.country_id' => 'nullable|exists:countries,id', // change here
            'addresses.*.city' => 'nullable|string|max:255',
            'addresses.*.state' => 'nullable|string|max:255',
            'addresses.*.postal' => 'nullable|string|max:20',
        ]);
                ...
                if ($request->has('addresses')) {
                    foreach ($request->addresses as $addressData) {
                        ...
                        $principal->addresses()->create([
                            ...
                            'state' => $addressData['state'] ?? null,
                            'postal' => $addressData['postal'] ?? null,
                            'country_id' => $addressData['country_id'] ?? null, // optional if you added the column
                        ]);
                    }
                }
        // Optional notes
