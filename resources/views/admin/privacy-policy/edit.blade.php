    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.privacy-policy.update', $privacyPolicy->id) }}" method="POST" id="privacyPolicyForm">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="card mb-3 section-item" id="section-{{ $index }}">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div id="section-editor-{{ $index }}" class="section-editor"></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ClassicEditor
            .create(document.querySelector('#section-editor-{{ $index }}'), {
                toolbar: {
                    items: [
                        'heading', '|',
            ...
            .then(editor => {
                sectionEditors[{{ $index }}] = editor;
                
                // Sync editor content with textarea
                editor.model.document.on('change:data', () => {
                    let content = editor.getData();
                    content = cleanHTMLContent(content);
                    document.querySelector('#section-content-{{ $index }}').value = content;
                });
                
                // Set existing section content
                const sectionContent = document.querySelector('#section-content-{{ $index }}').value;
                if (sectionContent) {
                    const cleanedContent = cleanHTMLContent(sectionContent);
                    editor.setData(cleanedContent);
                    document.querySelector('#section-content-{{ $index }}').value = cleanedContent;
                }
            ...
            .catch(error => {
                console.error('Failed to initialize section editor {{ $index }}:', error);
                // Fallback to textarea
                document.querySelector('#section-editor-{{ $index }}').style.display = 'none';
                document.querySelector('#section-content-{{ $index }}').classList.remove('d-none');
            });
    }, 100);
    @endforeach
